...
int lmshoot_usage(bool print_template_params)
{
  ...
  cout << "  -seed val                  : set random generator seed to val, default (val=0) is stochastic behavior" << endl;
  return -1;
}
...
template <class TFloat, unsigned int VDim>
std::mt19937 PointSetShootingProblem<TFloat, VDim>::m_Random;
    ...
    ::minimize(const ShootingParameters &param)
{
  ...
  m_Random.seed(param.random_seed == 0 ? std::chrono::system_clock::now().time_since_epoch().count() : param.random_seed);
  ...
  while(!cl.is_at_end())
    {
      ...
      if(arg == "-m")
        ...
        {
          param.random_seed = cl.read_integer();
        }
      else if(arg == "-h")
        {
